...
export function getPointsToNextTier(currentPoints: number, currentMembership: Membership | null, allTiers: Membership[]) {
  if (!currentMembership) {
    const firstTier = allTiers.find(tier => tier.minPoints === 0);
...
export function getProgressToNextTier(currentPoints: number, currentMembership: Membership | null, allTiers: Membership[]) {
  if (!currentMembership) {
    const firstTier = allTiers.find(tier => tier.minPoints === 0);
